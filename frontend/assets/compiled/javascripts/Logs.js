...
angular.module("BitriseAddonFirebaseApp").controller("LogsController", function($scope, $timeout, iframeService, requestService, pageDetailsService, Progress) {
	...
	function detectorMethodForAndroid(rawLogs) {
		return _.any(_.filter(rawLogs.split("\n"), function(aRawLogLine) {
			return aRawLogLine;
		}), function(aRawLogLine) {
			if (!aRawLogLine) {
				return false;
	...
	function detectorMethodForIos(rawLogs) {
		return _.any(_.filter(rawLogs.split("\n"), function(aRawLogLine) {
			...
			var regexp = new RegExp(/^(.*)  ([0-9]{0,2}) ([0-9]{0,2}):([0-9]{0,2}):([0-9]{0,2}) ([^\[]*)\[[0-9]*] <([^>]*)>: (.*)$/);
	...
	function logProcessMethodForIos(rawLogs) {
		...
		_.each(_.filter(rawLogs.split("\n"), function(aRawLogLine) {
			...
			try {
				...
				var month = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].indexOf(regexp.exec(aRawLogLine)[1]);
				var day = regexp.exec(aRawLogLine)[2];
				var hour = regexp.exec(aRawLogLine)[3];
				var minute = regexp.exec(aRawLogLine)[4];
				var second = regexp.exec(aRawLogLine)[5];
